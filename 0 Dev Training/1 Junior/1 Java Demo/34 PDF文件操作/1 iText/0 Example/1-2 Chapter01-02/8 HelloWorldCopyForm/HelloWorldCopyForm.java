...
public class HelloWorldCopyForm {
	...
	public static void main(String[] args) {
		...
		try {
			PdfReader reader = new PdfReader("c:/HelloWorldForm1.pdf");
			// step 1
			Document document = new Document(reader.getPageSizeWithRotation(1));
			// step 2
			PdfCopy writer = new PdfCopy(document, new FileOutputStream(
					"c:/HelloWorldCopyForm.pdf"));
			// step 3
			document.open();
			// step 4
			writer.addPage(writer.getImportedPage(reader, 1));
			reader = new PdfReader("c:/HelloWorldForm2.pdf");
			writer.addPage(writer.getImportedPage(reader, 1));
			reader = new PdfReader("c:/HelloWorldForm3.pdf");
			writer.addPage(writer.getImportedPage(reader, 1));
			// step 5
			document.close();
		} catch (IOException e) {
