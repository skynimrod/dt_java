...
public class HelloWorldSelectPages {
	...
	public static void main(String[] args) {
		...
		try {
			...
			document.close();
			reader = new PdfReader("c:/HelloWorldMultiplePages.pdf");
			reader.selectPages("e");
			pages = reader.getNumberOfPages();
			document = new Document();
			copy = new PdfCopy(document, new FileOutputStream(
					"c:/HelloWorldSelectPagesEven.pdf"));
			document.open();
			for (int i = 0; i < pages;) {
				++i;
				copy.addPage(copy.getImportedPage(reader, i));
			...
			document = new Document();
			copy = new PdfCopy(document, new FileOutputStream(
					"c:/HelloWorldSelectPages12379.pdf"));
			document.open();
			for (int i = 0; i < pages;) {
				++i;
				copy.addPage(copy.getImportedPage(reader, i));
